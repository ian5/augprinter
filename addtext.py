...
def printAllCards(start=-1,end=99999,mode=0,fmt=""):
    ...
    for index in cardRange:
        ...
        if sortOutput:
            ...
            printCard(cardInfo,prefix=confirmDirectory("output/"+TTSPath),fmt=fmt,liveUpdate=livePreview)
        else:
            printCard(cardInfo,prefix=confirmDirectory("output/reprint/"),fmt=fmt,liveUpdate=livePreview)
        counter+=1
    # end for
...
def printCard(info,savePath="output",show=False,prefix="01x 001 ",fmt="",liveUpdate=False):
    ...
    try:
        ...
        bgImg = Image.open("assets/bg/zerror.png")
    img.paste(bgImg,(13,27))
    bgImg.close()

    # It needs a photo...
    try:
        artImg = Image.open("assets/art/"+info["name"]+".png")
    except FileNotFoundError:
        print("could not find portrait: "+info["name"]+".png")
        artImg = Image.open("assets/art/nothing lol.png")
    # end try
    ...
    try:
        ...
        frameImg = Image.open("assets/frames/zerror.png")
    ...
    for ii in range(costn):
        ...
        string = i[1]
        try:
            costImg = Image.open("assets/cost/"+string+".png")
        except FileNotFoundError:
            try:
                string = string[:-1]
                costImg = Image.open("assets/cost/"+string+".png")
                ...
                costImg = Image.open("assets/cost/zerror.png")
            # end try
        # end try
        w = costImg.getbbox()[2]
        h = costImg.getbbox()[3]
        ...
        if i[0] < threshold:
            ...
            costx -= ((w-1)+6)
            alphaPaste(img,costx,costy+1,"assets/cost/x.png")
            costx-=6
            for j in range(len(numberString)):
                chrj = numberString[-j-1]
                symbolString = "assets/cost/"+chrj+".png"
    ...
    if info["traits"] != []:
        for i in info["traits"]:
            if "Power" in i:
                normalPower = False
                try:
                    alphaPaste(img,144,1351,"assets/variable/"+i.strip()+".png")
                except FileNotFoundError:
    ...
    if conduit and gem:
        alphaPaste(img,50,840,"assets/misc/gems/gemconduit.png")
        if gem=="!":
            gem=""
        # end if
        ...
        if "Prism Conduit" in info["sigils"]:
            ...
            gem = "S"
        # end if
        gemImg = "assets/misc/gems/gem"+gem+".png"
        alphaPaste(img,530,850,gemImg)
    elif gem:
        if gem == "P":
            alphaPaste(img,40,840,"assets/misc/gems/moxband_prism.png")
        elif gem == "S":
            alphaPaste(img,40,840,"assets/misc/gems/moxband_shard.png")
        else:
            alphaPaste(img,40,840,"assets/misc/gems/moxband_3empty.png")
            if "O" in gem:
                alphaPaste(img,440,850,"assets/misc/gems/gemO.png")
            # end if
            if "B" in gem:
                alphaPaste(img,530,850,"assets/misc/gems/gemB.png")
            # end if
            if "G" in gem:
                alphaPaste(img,610,850,"assets/misc/gems/gemG.png")
            # end if
        # end if
    elif conduit:
        alphaPaste(img,50,850,"assets/misc/conduit_large.png")
    # end if
    ...
    for isign in range(nsig):
        ...
        if isig in METASIGILS:
            if isig != "INFOBOX":
                alphaPaste(img,0,sigily,"assets/misc/"+isig+".png")
                sigily+=120
            # end if
            ...
            if isig in BOXSIGILS: # some day replace this with a proper check for if we add more like this
                ...
                darkBox = Image.open("assets/misc/META_DARKBOX.png")
                ...
                darkBox.close()
                darkBoxEnd = Image.open("assets/misc/META_DARKBOX_END.png")
        ...
        if specialSigil:
            if isig in COLORSIGILS:
                try:
                    sigilImg = recolorImage(Image.open("assets/sigils/"+isig+"_outline.png"),(255,255,255))
                except FileNotFoundError:
                    sigilImg = recolorImage(Image.open("assets/sigils/"+isig+".png"),(255,255,255))
                    print("No outline variant for sigil: "+isig)
                except FileNotFoundError:
                    sigilImg = recolorImage(Image.open("assets/sigils/zerror.png"),(255,255,255))
            ...
            try:
                sigilImg = Image.open("assets/sigils/"+isig+".png")
            except FileNotFoundError:
                sigilImg = Image.open("assets/sigils/zerror.png")
                print("No icon for sigil: "+isig)
            #end try
        #end if
    ...
    if info["sigils"] != [] and not dontPutTheExtraLineThere:
        alphaPaste(img,150,sigily-(sigily%10),"assets/misc/Separator_large.png")
        sigily += 10
    # end if
    ...
    for itrait in info["traits"]:
        if itrait in DECALTRAITS:
            ...
            alphaPaste(img,0,0,"assets/misc/"+itrait+".png")
            continue
        # end if
        hasTraits = True
    ...
    if hasTraits:
        ...
        alphaPaste(img,150,sigily,"assets/misc/Separator_large.png")
        sigily += 10
    # end if
